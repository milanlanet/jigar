...
/*** Create Timeline Page Post type ***/
        pm_register_post_type();

/*** Create Patient Page Post type ***/        
        p_register_post_type();

/*** Create Pages for both Post type ***/
        create_page_post();
}
...
function create_page_post() {
        ...
        foreach ($timeline_array as $k => $v) {
				...
				$i++;
                $exists = get_page_by_title( $v, OBJECT, 'timeline_page' );
                if($exists->ID>0 && is_numeric($exists->ID))
                {
		...
		foreach ($p_array as $k1 => $v1) {
				...
				$j++;
                $exists_patient_pages = get_page_by_title( $v1, OBJECT, 'patient_page' );
                if($exists_patient_pages->ID>0 && is_numeric($exists_patient_pages->ID))
                {
...
array('name' => 'steps-to-creating-template',
                'title' => 'Click here to see your page template choices',
                'show_in_timeline' => 0,
                'row_pos'=>2,
...
<p style="text-align: center;">After selection, click replace existing content</p>'),
        ...
        foreach ($ptemp_arr as $k => $v) {
                ...
                $exists_templates = get_page_by_title( $v['title'], OBJECT, 'patient_templates' );
                
                if($exists_templates->ID>0 && is_numeric($exists_templates->ID) && $exists_templates->post_status=="publish")
                {
...
function pm_template_enqueue_scripts() {
      ...
      //  wp_enqueue_script('tinymce_saved', 'cdn.tinymce.com/4/tinymce.min.js');
       wp_enqueue_style('custom_styles', plugins_url('css/cus_styles.css', __FILE__));
        ...
        wp_enqueue_style('owl_min_css',plugins_url('css/owl.carousel.min.css', __FILE__));
        wp_enqueue_style('owl_default_css',plugins_url('css/owl.theme.default.min.css', __FILE__));
       ...
       // wp_enqueue_script('cu_beave_script_121',plugins_url('js/fl-builder-layout.js', __FILE__));
        ...
        wp_enqueue_script('yui3', plugins_url('js/yui-min.js',__FILE__));
        wp_enqueue_script('owl-carousel-js', plugins_url('js/owl.carousel.min.js',__FILE__));
...
<script>
  ...
  jQuery(function() {
    ...
    jQuery( ".timeline_row" ).disableSelection();
  });
  </script>
  <?php
                if (!empty($posts)) {
                                                ...
                                                jQuery(document).ready(function () {
													jQuery('.mail_link').click(function(){
														...
														//	tinyMCE.get('mycustomeditor_afds').setContent("<?php echo addslashes(get_user_meta($current_user->ID,'int_msg',true)); ?><br>"+email_link+''+"<br><?php echo addslashes(get_user_meta($current_user->ID,'timeline_email',true)); ?>");
                                                          ...
                                                          jQuery('.subject').val("<?php echo get_user_meta($current_user->ID,'timeline_subject',true); ?>");
                                                        });
																		...
																		success:function(res)
																		{
																			...
																			if(res)
																			{
																				...
																				//	jQuery('#email_link_myModal').trigger('click'); 
																					jQuery('#email_link_myModal').modal('toggle');
                                                        ...
                                                        jQuery("#tree_<?php echo $v->ID; ?>").fancytree({
                                                                ...
                                                                switch (data.cmd) {
                                                                        ...
                                                                        case "remove":
																				...
																				if(node.hasChildren())
																				{
																					alert('Delete all children first');
																				}
																				else
																				{
																					node.remove();
                            ...
                            <div class="modal-transclude" ng-transclude="">
                                <div class="login-prompt ng-scope" ng-show="showLoginPrompt">    
                                    <div class="umtop">
                                        ...
                                        <div class="um_box_up pm_temp_title">Select template</div>
                                                                                ...
                                                                                jQuery.ajax({
																					...
																					success:function(res1){
																						...
																						jQuery.ajax({
																							url: "<?php echo admin_url('admin-ajax.php'); ?>",
																							type: 'POST',
																							data: 'action=insert_meta_value&pid=' + inserted_post + '&current_clicked_node=' + jQuery(".current_clicked_node").val() + '&title=' + title_field + '&content=' + cnt + '&cid=' + cid +'&is_timeline_added='+is_time_added,
...
function insert_meta_value(){
		 ...
		 if(isset($_POST['is_timeline_added']))
		 {
			 $timeline_add = empty($_POST['is_timeline_added']) ? 0 : $_POST['is_timeline_added'];
			 update_post_meta($_POST['pid'],'is_timeline_added',$_POST['is_timeline_added']);
         }
		 echo "changed";
	exit;
}
...
function pm_content_disp() {
        ...
        {
			$is_timeline_added = get_post_meta($post->ID,'is_timeline_added',true);
			if(isset($is_timeline_added) && !empty($is_timeline_added))
			{
				$arr['close_popup'] = 1;
				//$arr['content'] = do_shortcode($post->post_content);
				echo json_encode($arr);
				//echo json_encode($arr['close_popup']);
				exit;
			}
			else
			{
...
function page_timeline_shortcode($attr)
{
			...
			$cwidth=count($tree_time[0]['children'])*310;
			echo '----------------->'.count($tree_time[0]['children']);
echo '<div id="scrollbar"><a id="left_scroll" class="mouseover_left" href="#"></a><div id="track"><div id="dragBar"></div></div><a id="right_scroll" class="mouseover_right" href="#"></a></div></div>';
			echo '<div id="horiz_container_outer"><div id="horiz_container_inner"><div id="horiz_container"><div class="row timeline_row ui-sortable">';
                                                                                ...
                                                                                jQuery.ajax({
                                                                                        ...
                                                                                        success: function (res) {
																							...
																							if(res.title == null)
																								...
																								{
																									jQuery('.pm_popup_modal_back .title_back h5').text(res.title);
																								}
																								if(res.url == false)
																								{
	...
	?>
			  ...
			  <script>
				  ...
				  jQuery(document).ready(function(){
					  ...
					  setTimeout(function(){
						  jQuery('#timeline_email_ifr').contents().find('#tinymce').html("<?php echo addslashes(get_user_meta($current_user->ID,'timeline_email',true)); ?>");
						  jQuery('#timeline_email_wrap_ifr').contents().find('#tinymce').html("<?php echo addslashes(get_user_meta($current_user->ID,'int_msg',true)); ?>");
				  ...
				  jQuery('.save_details').click(function(e){
						...
						var signature = jQuery('#timeline_email_ifr').contents().find('#tinymce').html();
						var int_msg = jQuery('#timeline_email_wrap_ifr').contents().find('#tinymce').html();
